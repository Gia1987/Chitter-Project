...
describe User do
    describe '.create' do
      it 'creates a new user' do
        user = User.create(email: 'giacomo@example.com', password: 'password123')
    ...
    describe '.authenticate' do
      it 'returns a user given a correct username and password, if one exists' do
        user = User.create(email: 'giacomo@example.com', password: 'password123')
        authenticated_user = User.authenticate('giacomo@example.com', 'password123')
      ...
      it 'returns nil given an incorrect password' do
        ...
        expect(User.authenticate('giacomo@example.com', 'wrongpassword')).to be_nil
      end
    end
  end
